...
const Main: React.FC = () => {
  ...
  const handleInput = ({currentTarget: {value}}: React.SyntheticEvent<HTMLInputElement>) => {
    setTextSearch(value)
  }
  ...
  const scheduledEvents = useMemo((): SESchema[] => {
    ...
    const filtered = events.filter(
      ({title, historyEvents}) =>
        title.match(new RegExp(titleRegexValue, 'gi')) && hasHistoryEvents(historyEvents) && anyLateCheck(historyEvents),
    )
  ...
  return (
    <Container>
      ...
      <CustomInput className="my-4" placeholder="Filtruj po drużynie" type="text" value={textSearch} onChange={handleInput} />
